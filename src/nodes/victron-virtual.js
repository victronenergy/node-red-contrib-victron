    ...
    function instantiateDbus(self) {
      ...
      async function proceed(usedBus) {
        ...
        switch (config.device) {
          ...
          case 'switch': {
            // Legacy support: Existing Virtual Device (switch) nodes continue to work.
            // New users should use the dedicated "Virtual Switch" node instead.
            createSwitchProperties(config, ifaceDesc, iface)
            text = getSwitchStatusText(config)
            break
          }
          case 'tank':
