...
function main () {
  ...
  if (Object.keys(missingPaths).length) {
    ...
    Object.keys(missingPaths).forEach(key => {
      const pathObjs = {}
      const missingPathsArray = [...missingPaths[key]]
...
function parseCSVData (additionalData) {
  ...
  if (additionalData) { dataAttributes = _.merge(dataAttributes, additionalData) }

  const data = Object.assign({}, whitelist) // clone whitelist
    ...
    Object.entries(nodeServices).forEach(([dbusService, servicePaths]) => {
      data[nodeName][dbusService] = servicePaths.map(dbusPath => {
        ...
        if (attribute === undefined) {
          if (missingPaths[dbusService] === undefined) { missingPaths[dbusService] = new Set([dbusPath]) } else { missingPaths[dbusService].add(dbusPath) }
          return null
        }
